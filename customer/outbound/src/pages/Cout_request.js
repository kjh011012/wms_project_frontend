...
import { mockOutboundItems } from './mock/mockOutboundItems';
...
function Customeroutboundrequest() {
  ...
  try {
    ...
    const filtered = mockOutboundItems.filter(item =>
...
const handleRequestSubmit = async (id) => {
  ...
  try {
    // ✅ 실 API 호출 대신 mock 응답 사용
    const mockResponse = {
      비용차이: 5000,
      최종청구비용: selectedRow.total_cost + 5000,
      설명: "모의 출고 요청 완료 (mock 데이터 기반)",
    };

    alert(`(Mock) 출고 요청 완료\n\n비용 차이: ${mockResponse.비용차이}원\n최종 청구 비용: ${mockResponse.최종청구비용}원\n설명: ${mockResponse.설명}`);
    setIsModalOpen(false);
    fetchData(); // 리스트 새로고침
  } catch (error) {
    console.error('출고 요청 중 오류 (mock):', error);
    alert('출고 요청 중 오류가 발생했습니다. (mock)');
  }
};
