...
const Auth = () => {
  ...
  try {
    const { email, role } = loginUser(loginData.email, loginData.password);
    if (email && role) {
  ...
  if (!signupData.agreement) {
    return setErrorMessage("개인정보 보호 정책에 동의해야 합니다.");
  }
  ...
  try {
    const { email, role } = signupUser(userData);
    if (email && role) {
      localStorage.setItem("email", email);
      localStorage.setItem("role", role);
      navigate("/");
  ...
  // 테스트용 가짜 사용자 정보로 대체
  const decoded = { email: "googleuser@example.com", name: "구글유저" };
  try {
