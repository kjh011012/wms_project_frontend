...
import { mockDropdownData, mockTableData, mockImageData } from "./mock/mockInventoryData";
...
const InventoryStatus = () => {
  ...
  setDropdownData(mockDropdownData);
}, []);
  ...
  const filtered = mockTableData.filter((item) => {
    const matchesProduct = !searchFilters.productNumber || item.product_number.includes(searchText);
    const matchesDate = !searchFilters.arrivalDate || item.arrival_date?.includes(searchText);
    return matchesProduct && matchesDate;
  });
  setTableData(filtered.slice(0, rowsPerPage));
  setLoading(false);
}, [searchFilters, searchText, rowsPerPage]);


  const fetchProductImages = useCallback((productId) => {
  setImageData(mockImageData);
}, []);

  const fetchImageData = useCallback(() => {
  if (!checkboxValues.pallet && !checkboxValues.palletRack) return;
