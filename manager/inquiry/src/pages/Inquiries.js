...
function Inquiries() {
  ...
  if (searchText.trim()) {
    ...
    filtered = filtered.filter(
      (inquiry) =>
        ...
        inquiry.author_email.toLowerCase().includes(keyword) ||
  ...
  const newItem = { ...formData, date: newDate };

  if (showWritePanel) {
    const newId =
      mockInquiries.length > 0
        ? Math.max(...mockInquiries.map((i) => i.id)) + 1
        : 1;
    mockInquiries.push({ ...newItem, id: newId });
    alert("문의사항이 추가되었습니다.");
  } else if (selectedInquiry) {
    const index = mockInquiries.findIndex((i) => i.id === selectedInquiry.id);
    if (index !== -1) {
      mockInquiries[index] = { ...selectedInquiry, ...newItem };
      alert("문의사항이 수정되었습니다.");
    }
  }
