...
const fetchTableData = async () => {
  try {
    console.log("📦 MOCK 데이터를 불러옵니다.");
    setTableData(MOCK_INBOUND_STATUS);
  } catch (error) {
  ...
  try {
    ...
    const index = MOCK_INBOUND_STATUS.findIndex(
    ...
    MOCK_INBOUND_STATUS[index] = {
      ...MOCK_INBOUND_STATUS[index],
      ...updatedData
    };

    alert("견적이 성공적으로 업데이트되었습니다! (Mock)");
    fetchTableData(); // 데이터 새로고침
    setIsCalculatorOpen(false);
    setSelectedRowData(null);
  } catch (error) {
    console.error("MOCK 업데이트 실패:", error);
    alert("업데이트 실패");
  }
};
