...
function Min_contract_state_detail({ contract, isOpen, onClose }) {
  ...
  // 계약서 폼 데이터 가져오기 mock 데이터 사용
  const fetchContractForm = async (contractId) => {
  try {
    const data = MOCK_CONTRACT_DETAIL.find(item => item.id === contractId);
    if (!data) throw new Error('Mock 데이터에서 계약을 찾을 수 없습니다.');
    ...
    setSelectedRowData(data); // 실견적 계산기 등에서 사용
  } catch (error) {
  ...
  useEffect(() => {
    if (contract?.id) {
      ...
      setBarcodeUrl(`/mock/barcode_${contract.id}.png`); // public/mock/barcode_1.png 등
  ...
  try {
    const targetIndex = MOCK_CONTRACT_DETAIL.findIndex(item => item.id === cid);
    if (targetIndex === -1) {
    ...
    // mock 데이터 수정
    MOCK_CONTRACT_DETAIL[targetIndex] = {
      ...MOCK_CONTRACT_DETAIL[targetIndex],
      ...contractForm
    };

    console.log("💾 Mock 계약서 저장 완료:", MOCK_CONTRACT_DETAIL[targetIndex]);
    alert('계약서가 저장되었습니다. (Mock)');
    setIsEditable(false);
