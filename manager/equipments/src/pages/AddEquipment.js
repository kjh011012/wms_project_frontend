...
import mockUserPhoneData from "./mock/mockUserPhoneData";
...
const AddEquipment = ({ onClose, onAdd }) => {
  ...
  const matched = mockUserPhoneData.filter(
    user => user.username.toLowerCase() === name.toLowerCase()
  );

  if (matched.length === 1) {
    setPhoneOptions([]);
    setFormData(prev => ({ ...prev, assigned_to_phone: matched[0].number }));
  } else if (matched.length > 1) {
    setPhoneOptions(matched);
    setFormData(prev => ({ ...prev, assigned_to_phone: "" }));
  } else {
    setPhoneOptions([]);
    setFormData(prev => ({ ...prev, assigned_to_phone: "" }));
  ...
  try {
    const newItem = {
      ...formData,
      id: mockEquipmentData.length + 1,
      created_at: new Date().toISOString().split("T")[0],
    };

    mockEquipmentData.push(newItem); // ✅ mock 배열에 추가
    onAdd();   // 부모에 알림 → 테이블 리프레시
    onClose(); // 모달 닫기
  } catch (error) {
    console.error("Mock 추가 실패:", error);
  }
};
